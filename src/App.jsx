...
const Tag = ({ children, color = "#64748b", active, onClick }) => (
  <button onClick={onClick} style={{ padding: "4px 9px", borderRadius: 5, fontSize: 28, fontFamily: F, background: active ? `${color}20` : "transparent", border: `1px solid ${active ? color + "50" : "#1e293b"}`, color: active ? color : "#64748b", cursor: "pointer", transition: "all 0.15s" }}>{children}</button>
);
const StatMini = ({ l, v, c }) => <div style={{ textAlign: "center" }}><div style={{ fontSize: 22, color: "#475569", fontFamily: F }}>{l}</div><div style={{ fontSize: 22, color: c, fontWeight: 700, fontFamily: F }}>{v}</div></div>;
const StatBar = ({ label, value, max, color }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
    <span style={{ width: 28, fontSize: 22, color: "#64748b", fontFamily: F }}>{label}</span>
    ...
    <span style={{ width: 22, fontSize: 14, color: "#e2e8f0", fontFamily: F, textAlign: "right", fontWeight: 700 }}>{value}</span>
  </div>
);
  ...
  return (
    <div onClick={onClick} style={{ width: w, maxWidth: "100%", height: h, borderRadius: 12, background: "#0a0e27", border: `2px solid ${el.color}35`, boxShadow: `0 0 20px ${el.color}10`, cursor: onClick ? "pointer" : "default", overflow: "hidden", transition: "all 0.2s", flexShrink: 0, position: "relative" }}
      ...
      {card.ability && lg && <div style={{ margin: "2px 10px 8px", padding: "5px 7px", borderRadius: 5, background: `${el.color}0d`, border: `1px solid ${el.color}15` }}>
        <div style={{ fontFamily: F, fontSize: 22, color: el.color, fontWeight: 700 }}>{card.ability}</div>
        <div style={{ fontFamily: F, fontSize: 22, color: "#94a3b8" }}>{card.abilityDesc}</div>
      </div>}
    </div>
  );
}
...
export default function WhalemonTCG() {
  ...
  const loadWhales = async () => {
    ...
    try {
      ...
      const whaleCards = new Contract(CONTRACTS.WHALE_CARDS, WHALE_CARDS_ABI, prov);
      const bal = await whel.balanceOf(walletAddr);
      const count = Number(bal);
      console.log("WHEL balance:", count);
      const whales = [];
      ...
      for (let i = 0; i < count; i++) {
        try {
          ...
          let hasMinted = false;
          try { hasMinted = await whaleCards.hasMinted(id); } catch(e) {}
          if (hasMinted) minted.add(id);
          whales.push({ id, image: nftImg(id), traits: {} });
        } catch (e) { console.log("Whale token load error:", e); }
      }
      setMyWhales(whales);
      setMintedIds(minted);
      if (count === 0) notify("No WHEL NFTs found. Make sure you're on Tempo Network!", "info");
    } catch (e) {
      console.log("Load whales error:", e);
      notify("Error loading NFTs: " + (e.reason || e.message || "Unknown"), "error");
    }
...
@media(max-width:768px){.wm-market-layout{flex-direction:column}.wm-lb-grid{grid-template-columns:32px 1fr 44px 44px 44px 44px;padding:8px 8px;font-size:9px}}`;
  ...
  if (!connected) return (
    <div style={{ minHeight: "100vh", background: "#030712", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", fontFamily: F }}>
      ...
      <div style={{ position: "relative", zIndex: 1, textAlign: "center", animation: "slideUp .8s ease" }}>
        ...
        <h1 style={{ fontSize: 48, fontWeight: 800, background: "linear-gradient(135deg,#38bdf8,#818cf8,#38bdf8)", backgroundSize: "200%", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "shimmer 3s linear infinite", letterSpacing: -1 }}>WHALEMON</h1>
        <div style={{ fontSize: 22, color: "#64748b", letterSpacing: 6, textTransform: "uppercase", marginBottom: 32 }}>Trading Card Game</div>
        ...
        <Btn onClick={handleConnect} style={{ padding: "14px 44px", fontSize: 28, letterSpacing: 1, boxShadow: "0 4px 28px rgba(14,165,233,.3)" }}>Connect Wallet</Btn>
        <div style={{ marginTop: 8, fontSize: 28, color: "#475569" }}>Tempo Network · Gas in PATHUSD</div>
        <div style={{ marginTop: 44, display: "flex", gap: 24, justifyContent: "center" }}>{[["3,333", "Whales"], ["6", "Elements"], ["∞", "Battles"]].map(([v, l], i) => <div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 800, color: "#e2e8f0" }}>{v}</div><div style={{ fontSize: 22, color: "#64748b", textTransform: "uppercase", letterSpacing: 2 }}>{l}</div></div>)}</div>
      </div>
    </div>
  );

  const Nav = ({ l, ic, id }) => <button onClick={() => { setPage(id); setSelectedCard(null); }} style={{ padding: "8px 14px", borderRadius: 7, border: page === id ? "1px solid #0ea5e922" : "1px solid transparent", background: page === id ? "#0ea5e90d" : "transparent", color: page === id ? "#38bdf8" : "#64748b", fontFamily: F, fontSize: 14, fontWeight: page === id ? 700 : 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, transition: "all .15s" }}><span style={{ fontSize: 13 }}>{ic}</span>{l}</button>;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030712", fontFamily: F, position: "relative" }}>
      <style>{css}</style><Bubbles />
      {notif && <div style={{ position: "fixed", top: 14, right: 14, zIndex: 1000, padding: "9px 16px", borderRadius: 7, background: notif.t === "info" ? "#1e3a5f" : notif.t === "error" ? "#450a0a" : "#064e3b", border: `1px solid ${notif.t === "info" ? "#0ea5e922" : notif.t === "error" ? "#f8717122" : "#4ade8022"}`, color: "#e2e8f0", fontSize: 14, fontFamily: F, animation: "notifSlide .25s", boxShadow: "0 4px 20px rgba(0,0,0,.4)", maxWidth: 300 }}>{notif.m}</div>}
      ...
      {revealPhase && <div style={{ position: "fixed", inset: 0, zIndex: 900, background: "rgba(3,7,18,.93)", backdropFilter: "blur(14px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "fadeIn .3s" }}>
        ...
        {(revealPhase === "minting" || revealPhase === "generating") && <div style={{ textAlign: "center", animation: "slideUp .4s" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0" }}>{revealPhase === "minting" ? "Minting on Tempo..." : "Oracle Generating Stats..."}</div>
          <div style={{ fontSize: 28, color: "#64748b", marginTop: 3 }}>{revealPhase === "minting" ? "Confirm in MetaMask" : "AI analyzing your whale traits"}</div>
        </div>}
        {(revealPhase === "revealing" || revealPhase === "done") && revealCard && <div style={{ textAlign: "center" }}>
          ...
          {revealPhase === "done" && <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: ELEMENTS[revealCard.element]?.color, animation: "slideUp .4s ease .2s both" }}>{ELEMENTS[revealCard.element]?.icon} {ELEMENTS[revealCard.element]?.name} {RARITIES[revealCard.rarity]}!</div>
      ...
      {showOffer && <div style={{ position: "fixed", inset: 0, zIndex: 800, background: "rgba(3,7,18,.9)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", animation: "fadeIn .2s" }}>
        <div style={{ background: "#0f172a", borderRadius: 14, border: "1px solid #1e293b", padding: 20, width: 340, maxWidth: "calc(100vw - 32px)" }}>
          ...
          <div style={{ marginBottom: 12 }}><span style={{ fontSize: 28, color: "#64748b" }}>Offer (PATHUSD):</span><input type="number" value={offerAmt} onChange={e => setOfferAmt(e.target.value)} placeholder="0.00" style={{ width: "100%", marginTop: 3, padding: "9px 10px", borderRadius: 6, background: "#0a0e27", border: "1px solid #1e293b", color: "#e2e8f0", fontFamily: F, fontSize: 28, outline: "none" }} /><span style={{ fontSize: 22, color: "#475569" }}>Balance: {walletBalance} PATHUSD</span></div>
      ...
      <header style={{ position: "relative", zIndex: 10, padding: "10px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #0ea5e907", background: "rgba(3,7,18,.85)", backdropFilter: "blur(10px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 20 }}>🐋</span><span style={{ fontSize: 28, fontWeight: 800, color: "#e2e8f0" }}>WHALEMON</span><span style={{ fontSize: 22, color: "#0ea5e9", letterSpacing: 2 }}>TCG</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <div style={{ padding: "4px 8px", borderRadius: 5, background: "#0ea5e907", border: "1px solid #0ea5e915", fontSize: 22, color: "#38bdf8", display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 4, height: 4, borderRadius: "50%", background: "#4ade80" }} />Tempo</div>
          <div style={{ padding: "4px 8px", borderRadius: 5, background: "#111827", border: "1px solid #1e293b", fontSize: 14, color: "#4ade80", fontWeight: 700 }}>${walletBalance} <span style={{ color: "#475569", fontWeight: 400 }}>USD</span></div>
          <div style={{ padding: "4px 8px", borderRadius: 5, background: "#111827", border: "1px solid #1e293b", fontSize: 28, color: "#e2e8f0" }}>{walletAddr.slice(0, 6)}...{walletAddr.slice(-4)}</div>
          <button onClick={handleDisconnect} style={{ padding: "4px 8px", borderRadius: 5, background: "transparent", border: "1px solid #dc262628", fontSize: 22, color: "#f87171", cursor: "pointer", fontFamily: F }}>Disconnect</button>
        </div>
      </header>
      ...
      <main style={{ position: "relative", zIndex: 10, padding: 18, maxWidth: 1180, margin: "0 auto", animation: "fadeIn .3s" }}>
        ...
        {page === "whales" && <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div><h2 style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0", marginBottom: 3 }}>My Whales</h2><p style={{ fontSize: 14, color: "#64748b" }}>Generate a Whalemon card from each WHEL NFT — free, one per whale.</p></div>
          ...
          {!loadingWhales && myWhales.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#475569", fontSize: 10 }}>
            ...
            <div style={{ fontSize: 22, marginTop: 4, color: "#334155" }}>You need WHEL NFTs to generate Whalemon cards.</div>
          </div>}
          <div className="wm-cards-grid">{myWhales.map(w => <div key={w.id} style={{ width: 180, maxWidth: "calc(50% - 8px)", minWidth: 150, borderRadius: 10, background: "#0f172a", border: "1px solid #1e293b", overflow: "hidden", transition: "all .2s", position: "relative" }} onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"} onMouseLeave={e => e.currentTarget.style.transform = "none"}>
            ...
            <div style={{ position: "absolute", top: 5, left: 5, padding: "2px 6px", borderRadius: 4, background: "rgba(0,0,0,.6)", fontSize: 28, color: "#e2e8f0", fontWeight: 700 }}>#{w.id}</div>
            <div style={{ padding: 8 }}>
              {mintedIds.has(w.id) ? <div style={{ padding: "5px 0", borderRadius: 5, background: "#065f460d", border: "1px solid #065f4620", textAlign: "center", fontSize: 28, color: "#4ade80" }}>✓ Card Minted</div>
                : <Btn small onClick={() => handleMint(w.id)} disabled={minting === w.id} style={{ width: "100%" }}>{minting === w.id ? "Minting..." : "⚡ Generate Card"}</Btn>}
            </div>
            {minting === w.id && <div style={{ position: "absolute", inset: 0, background: "rgba(3,7,18,.85)", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6 }}><div style={{ width: 24, height: 24, border: "2px solid #0ea5e928", borderTop: "2px solid #0ea5e9", borderRadius: "50%", animation: "spin 1s linear infinite" }} /><span style={{ fontSize: 22, color: "#38bdf8" }}>Minting...</span></div>}
          </div>)}</div>
        </div>}
        ...
        {page === "cards" && <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div><h2 style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0", marginBottom: 3 }}>My Cards</h2><p style={{ fontSize: 14, color: "#64748b" }}>Your battle-ready cards. Click to inspect.</p></div>
          ...
          <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
            ...
            {selectedCard && <div style={{ width: 300, maxWidth: "100%", flexShrink: 0, position: "sticky", top: 18, alignSelf: "flex-start", animation: "slideUp .3s" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 22, color: "#94a3b8", fontWeight: 700 }}>Details</span><button onClick={() => setSelectedCard(null)} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: 14 }}>✕</button></div>
              <GameCard card={selectedCard} size="large" />
              <div style={{ marginTop: 8, padding: 10, borderRadius: 8, background: "#0f172a", border: "1px solid #1e293b" }}><div style={{ fontSize: 22, color: "#64748b" }}>TOTAL POWER</div><div style={{ fontSize: 28, fontWeight: 800, color: "#e2e8f0" }}>{selectedCard.attack + selectedCard.defense + Math.floor(selectedCard.health / 3) + selectedCard.speed}</div></div>
            </div>}
          </div>
        </div>}
        ...
        {page === "battle" && <div>
          {!battleState && <div style={{ textAlign: "center", paddingTop: 28, animation: "slideUp .4s" }}>
            <div style={{ fontSize: 60, marginBottom: 14 }}>⚔️</div><h2 style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0", marginBottom: 5 }}>Battle Arena</h2>
            <p style={{ fontSize: 14, color: "#64748b", maxWidth: 380, margin: "0 auto 18px" }}>Choose your battle mode. Ranked matches cost 1 PATHUSD entry.</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              {[{ m: "free", ic: "🎯", t: "Practice", d: "vs AI · Free", bc: "#22c55e30", hc: "#22c55e" }, { m: "ranked-ai", ic: "🤖", t: "Ranked AI", d: "vs AI · 1 PATHUSD", bc: "#0ea5e930", hc: "#0ea5e9" }, { m: "pvp", ic: "👥", t: "Ranked PvP", d: "vs Player · 1 PATHUSD", bc: "#8b5cf630", hc: "#8b5cf6" }].map(x => <button key={x.m} onClick={() => startBattle(x.m)} style={{ padding: "18px 24px", borderRadius: 10, background: "#0f172a", border: `1px solid ${x.bc}`, width: 165, cursor: "pointer", textAlign: "center", transition: "all .2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = x.hc; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = x.bc; e.currentTarget.style.transform = "none"; }}>
                <div style={{ fontSize: 22, marginBottom: 4 }}>{x.ic}</div><div style={{ fontSize: 22, fontWeight: 700, color: "#e2e8f0", fontFamily: F }}>{x.t}</div><div style={{ fontSize: 22, color: "#64748b", fontFamily: F, marginTop: 1 }}>{x.d}</div>
              </button>)}
            </div>
          </div>}
          {battleState === "select" && <div style={{ animation: "slideUp .3s" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}><div><h2 style={{ fontSize: 28, fontWeight: 800, color: "#e2e8f0" }}>Choose Your Whalemon</h2><p style={{ fontSize: 28, color: "#64748b" }}>{battleMode === "free" ? "Practice (free)" : "Ranked (1 PATHUSD)"}</p></div><Btn small onClick={exitBattle} bg="#1e293b" style={{ border: "1px solid #334155", color: "#94a3b8" }}>← Back</Btn></div>
          ...
          {battleState === "fighting" && pCard && oCard && <div style={{ animation: "fadeIn .3s" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><span style={{ fontSize: 14, color: "#64748b" }}>Turn {bTurn} · {battleMode === "free" ? "Practice" : battleMode === "ranked-ai" ? "Ranked AI" : "PvP"}</span><Btn small onClick={exitBattle} bg="#1e293b" style={{ border: "1px solid #334155", color: "#94a3b8" }}>{bResult ? "Exit" : "Forfeit"}</Btn></div>
            <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
              {[{ lbl: "YOUR WHALEMON", c: pCard, clr: "#4ade80" }, null, { lbl: battleMode === "pvp" ? "OPPONENT" : "AI", c: oCard, clr: "#f87171" }].map((s, i) => {
                ...
                return <div key={i} style={{ flex: "1 1 250px", maxWidth: 300 }}>
                  <div style={{ textAlign: "center", marginBottom: 5, fontSize: 28, color: s.clr, fontWeight: 700 }}>{s.lbl}</div>
                  <div style={{ background: "#0f172a", borderRadius: 10, border: `1px solid ${ELEMENTS[cc.element]?.color || "#1e293b"}20`, padding: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>#{cc.id}</span><span style={{ fontSize: 22, color: ELEMENTS[cc.element]?.color }}>{ELEMENTS[cc.element]?.icon} {ELEMENTS[cc.element]?.name}</span></div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 6, overflow: "hidden" }}><CardImg src={cc.image} el={cc.element} h={40} /></div>
                      <div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 22, color: "#94a3b8", marginBottom: 2 }}><span>HP</span><span>{hp}/{cc.health}</span></div>
            ...
            {bResult && <div style={{ textAlign: "center", marginTop: 16, padding: "14px 24px", borderRadius: 10, background: bResult === "win" ? "#064e3b" : bResult === "lose" ? "#450a0a" : "#1e293b", border: `1px solid ${bResult === "win" ? "#4ade8028" : bResult === "lose" ? "#f8717128" : "#64748b28"}`, animation: "slideUp .3s" }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: bResult === "win" ? "#4ade80" : bResult === "lose" ? "#f87171" : "#94a3b8", marginTop: 3 }}>{bResult === "win" ? "VICTORY!" : bResult === "lose" ? "DEFEATED" : "DRAW"}</div>
            ...
            <div style={{ marginTop: 14, padding: 10, borderRadius: 8, background: "#0a0e27", border: "1px solid #1e293b", maxHeight: 140, overflowY: "auto" }}>
              <div style={{ fontSize: 22, color: "#475569", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>Battle Log</div>
              {bLog.map((l, i) => <div key={i} style={{ fontSize: 28, padding: "1px 0 1px 5px", color: logColor(l.tp), borderLeft: `2px solid ${logColor(l.tp)}20` }}>{l.txt}</div>)}
            </div>
          </div>}
        </div>}
        ...
        {page === "market" && <div style={{ animation: "slideUp .3s" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div><h2 style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0" }}>Marketplace</h2><p style={{ fontSize: 28, color: "#64748b" }}>Trade Whalemon cards in PATHUSD</p></div>
          </div>
          <div style={{ textAlign: "center", padding: 40, color: "#475569", fontSize: 10 }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>🏪</div>
            <div style={{ fontSize: 22, color: "#64748b", marginBottom: 4 }}>Marketplace Coming Soon</div>
            ...
            <div style={{ marginTop: 16, padding: "8px 14px", borderRadius: 8, background: "#0f172a", border: "1px solid #1e293b", display: "inline-block" }}>
              <div style={{ fontSize: 22, color: "#475569" }}>Contract</div>
              <div style={{ fontSize: 28, color: "#38bdf8" }}>{CONTRACTS.MARKETPLACE.slice(0, 10)}...{CONTRACTS.MARKETPLACE.slice(-6)}</div>
            </div>
          </div>
        </div>}
        ...
        {page === "leaderboard" && <div style={{ animation: "slideUp .3s" }}>
          <h2 style={{ fontSize: 22, fontWeight: 800, color: "#e2e8f0", marginBottom: 3 }}>Leaderboard</h2>
          <p style={{ fontSize: 14, color: "#64748b", marginBottom: 14 }}>Top Whalemon commanders</p>
          <div style={{ textAlign: "center", padding: 40, color: "#475569", fontSize: 10 }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>🏆</div>
            <div style={{ fontSize: 22, color: "#64748b", marginBottom: 4 }}>Season 1 Starting Soon</div>
            <div style={{ fontSize: 9 }}>Battle records will appear here once ranked matches begin.</div>
          </div>
        </div>}

      </main>
      <footer style={{ position: "relative", zIndex: 10, padding: "14px 18px", marginTop: 28, borderTop: "1px solid #0ea5e905", display: "flex", justifyContent: "space-between", fontSize: 22, color: "#1e293b" }}><span>Whalemon TCG</span><span>Tempo · PATHUSD · Chain 4217</span></footer>
    </div>
  );
}
